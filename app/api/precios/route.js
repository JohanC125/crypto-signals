...
export async function GET(request) {
  ...
  try {
    ...
    const id = ids[symbol];
    const res = await fetch(
      `https://api.coingecko.com/api/v3/simple/price?ids=${id}&vs_currencies=usd&include_24hr_change=true&include_high_24h=true&include_low_24h=true&include_24hr_vol=true`
    );
    const data = await res.json();
    return Response.json({
      ...
      volume: parseFloat((data[id].usd_24h_vol || 0).toFixed(2)),
    });
  } catch (e) {}
